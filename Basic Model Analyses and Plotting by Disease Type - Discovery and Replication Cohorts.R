setwd("U://Datastore//IGMM//marioni-lab//Rob//Epigenetic Clocks WHO")
...
for(j in 1:length(clock)){
  ...
  model <- summary(coxph(Surv(tte, event) ~ scale(d1[,clock[j]]) + age + sex, data=d1))
...
p = ggplot(test,aes(y=as.numeric(test$HR), x=test$Clock)) + 
  geom_point(position=position_dodge(width=0.5), size = 3, colour = "gold") +
  ...
  ggtitle("Survival - Replication") +
  theme(plot.title = element_text(hjust = 0.5)) + scale_y_continuous(limits = c(0.5,2.3)) +
  coord_flip()
...
for(j in 1:length(clock)){
  for(i in 1:length(categ)){
    ...
    clk <- clock[j]
    var = categ[i]
    p = summary(glm(d1[,categ[i]] ~ scale(d1[,clock[j]]) + age+ sex, data=d1, family="binomial"))$coefficients[2,4]
    n = nobs(glm(d1[,categ[i]] ~ scale(d1[,clock[j]])  + age + sex, data=d1, family="binomial"))
    n_events = table(d1[,categ[i]])[2]
    logodds <- summary(glm(d1[,categ[i]] ~ scale(d1[,clock[j]]) + age + sex , data=d1, family="binomial"))$coefficients[2,1]
    se <- summary(glm(d1[,categ[i]] ~ scale(d1[,clock[j]])  + age + sex, data=d1, family="binomial"))$coefficients[2,2]
...
out_cat$P <- as.numeric(out_cat$P)

## Linear Regression Analysis of Continuous Variables 


cont <- c("SIMD", "Neuroticism", "Systolic Pressure", "Forced Expiratory Flow", "Forced Expiratory Volume", "Forced Vital Capacity", 
...
for(j in 1:length(clock)){
  for(i in 1:length(cont)){
    clk = clock[j]
    var = cont[i]
    coeft = summary(lm(scale(d1[,cont[i]]) ~ scale(d1[,clock[j]]) + age + sex, data=d1))$coefficients[2,c(1,2,4)]
    n = nobs(lm(scale(d1[,cont[i]]) ~ scale(d1[,clock[j]]) + age + sex, data=d1))
...
for(j in 1:length(clock)){
  for(i in 1:length(cont)){
    clk = clock[j]
    var = cont[i]
    coeft = summary(lm(scale(d1[,cont[i]]) ~ scale(d1[,clock[j]]) + age, data=d1))$coefficients[2,c(1,2,4)]
    n = nobs(lm(scale(d1[,cont[i]]) ~ scale(d1[,clock[j]]) + age, data=d1))
...
out_cont$P <- as.numeric(out_cont$P)
...
## Alzheimers Disease 
ad_con = basic_con[which(basic_con$Variable %in% "g"),] 
...
ad$Variable = factor(ad$Variable, levels=c("g", "Alzheimers Disease Maternal", "Alzheimers Disease Paternal"))
